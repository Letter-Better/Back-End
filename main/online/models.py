...
class Room(models.Model):
    ...
    creator = models.ForeignKey(User, verbose_name="Creator", db_column="creator", on_delete=models.CASCADE,
                                error_messages=MESSAGES)
    time_of_draw = models.IntegerField(verbose_name="Time", db_column="time", choices=Time.choices,
                                       default=Time.thirty_sec, error_messages=MESSAGES)
    round = models.IntegerField(verbose_name="round", db_column="round", choices=Round.choices, default=Round.Two,
                                error_messages=MESSAGES)
    number_of_users = models.IntegerField(verbose_name="number of user", db_column="number_of_users",
                                          choices=NumberOFUsers.choices, default=NumberOFUsers.Three,
                                          error_messages=MESSAGES)
    room_type = models.IntegerField(verbose_name="Room Type", db_column="room_type", choices=RoomType.choices,
                                    default=RoomType.Public, error_messages=MESSAGES)
    difficulty = models.CharField(verbose_name="Difficulty", db_column="difficulty", choices=DIFFICULTY, default='easy',
                                  max_length=50, error_messages=MESSAGES)
    room_code = models.CharField(verbose_name="Room Code", db_column="room_code", max_length=30, unique=True,
                                 default=random_room_code(), error_messages=MESSAGES)
...
class RoomMember(models.Model):
    room = models.OneToOneField(Room, verbose_name="Room", db_column="room", related_name="roommember",
                                on_delete=models.CASCADE, error_messages=MESSAGES)
...
class OnlineRanking(models.Model):
    user = models.OneToOneField(User, verbose_name="User", db_column="user", on_delete=models.CASCADE,
                                error_messages=MESSAGES)
